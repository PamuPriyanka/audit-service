...
public class AuditController {
    ...
    public ResponseEntity<Page<AuditEntity>> search(@RequestParam(required = false) String actorId, @RequestParam(required = false) String resourceType, @RequestParam(required = false) Long resourceId, @RequestParam(required = false) String eventType, @RequestParam(required = false) LocalDateTime from, @RequestParam(required = false) LocalDateTime to, @PageableDefault(size = 20, sort = "id", direction = Sort.Direction.ASC) Pageable pageable) {
        return ResponseEntity.ok(auditService.searchAuditEvents(actorId, resourceType, resourceId, eventType, from, to, pageable));
    }

    @GetMapping("/verifyEventsChain")
